@php
    ...
    $customers = DB::table('customers')->select('id','customer_name','customer_address','customer_contact')->get();
    
    $hold_option ='';
    $customer_option ='';
    $option ='';
    $sale_record = DB::table('Sale_records')
                        ->select('sale_code','product_code','quantity','unit_price','discount','amount','created_at')
                        ->get();
    ...
    foreach ($customers as $item){
        $customer_option .= '<option value="'. $item->id .'">'. $item->customer_name .' - '.$item->customer_address.' - '.$item->customer_contact. '</option>';
    }
    ...
    <script>
        ...
        $('document').ready(function () {
            ...
            if(id!=""){
                ...
                var customer_id = sale[0]['customer_code'];
                ...
                $('#customer_address').val(value[0]['customer_address']);
                $('#customer_contact').val(value[0]['customer_contact']);
                ...
                var sale_code = sale_record.filter( function(sale_record){return (sale_record.sale_code==id);} );
                ...
                for(i=1 ; i <= default_row ; i++){
                    ...
                    var value = sale_code[i-1]['product_code'];
            ...
            function change(){
                $('.product_name').change(function(){
                    ...
                    $("#sale_tbody tr").each(function () {
                        var row = $(this);
                        var sd = {};
                        sd.product_code = row.find("td:eq(1) select").val();
                        if(sd.product_code > 0){
                    ...
                    @php $sale_price = DB::table('Product_stocks')->select('sale_price','product_code')->get(); @endphp;
                    var newPrice = @php echo $sale_price @endphp;
                    var getPrice = newPrice.filter( function(newPrice){return (newPrice.product_code==id);} );
            ...
            $('#btn-hold').on('click',function(){
                ...
                $("#sale_tbody tr").each(function () {
                    ...
                    if(sd.product_code > 0 && sd.price > 0 && sd.quantity > 0){
                        sale_records.push(sd);
                    }
                });
            ...
            $('#payment').on('click', function(){
                ...
                $("#sale_tbody tr").each(function () {
                    ...
                    if(sd.product_code > 0 && sd.price <=0){
                        check = false;
                    }
                });
                if(customer_name == ""){
